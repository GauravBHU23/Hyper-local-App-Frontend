...
export function ServiceCard({ provider, compact = false }: Props) {
  ...
  return (
    <div className={cn(
      "card hover:shadow-md hover:border-orange-100 transition-all duration-200 group",
      compact ? "p-3" : "p-4"
    )}>
      <Link href={`/services/${provider.id}`} className="block">
      ...
      {!compact && (provider.whatsapp || provider.phone) && (
